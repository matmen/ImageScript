...
export default class framebuffer {
  ...
  get(x, y) { return this.view.getUint32((~~x - 1) + (~~y - 1) * this.width, false); }
  toJSON() { return { width: this.width, height: this.height, buffer: Array.from(this.u8) } }
  overlay(frame, x = 0, y = 0) { return (ops.overlay.overlay(this, frame, ~~x, ~~y), this); }
  replace(frame, x = 0, y = 0) { return (ops.overlay.replace(this, frame, ~~x, ~~y), this); }
  set(x, y, color) { this.view.setUint32((~~x - 1) + (~~y - 1) * this.width, color, false); }
  scale(type, factor) { return this.resize(type, factor * this.width, factor * this.height); }
  at(x, y) { const offset = 4 * ((~~x - 1) + (~~y - 1) * this.width); return this.u8.subarray(offset, 4 + offset); }
